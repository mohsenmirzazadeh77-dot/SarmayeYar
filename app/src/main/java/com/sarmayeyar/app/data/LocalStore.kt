...
class LocalStore(context: Context) {
    private val prefs = context.getSharedPreferences("sarmayeyar", Context.MODE_PRIVATE)
    ...
    fun saveAssets(assets: List<Asset>) {
        ...
        prefs.edit().putString("assets", arr.toString()).apply()
    }
    ...
    fun loadHistory(): List<HistoryPoint> {
        ...
        return buildList {
            for (i in 0 until arr.length()) {
                val o = arr.getJSONObject(i)
                add(HistoryPoint(o.optLong("timestamp"), o.optLong("totalToman")))
            }
        }
    }
    ...
    fun saveHistory(history: List<HistoryPoint>) {
        val arr = JSONArray()
        history.takeLast(365).forEach { p ->
            arr.put(JSONObject().apply {
                put("timestamp", p.timestamp)
                put("totalToman", p.totalToman)
            })
        }
    ...
    fun exportJson(): String {
        ...
        root.put("assets", prefs.getString("assets", "[]"))
        root.put("history", prefs.getString("history", "[]"))
    ...
    fun importJson(json: String) {
        ...
        prefs.edit()
            .putString("assets", root.optString("assets", "[]"))
            .putString("history", root.optString("history", "[]"))
            .apply()
    }
}
